...
def demo(**config):
    ...
    json_file_path = {k: v["json_readme_path"] for k, v in config["keywords"].items()}
    md_readme_path = {k: v["md_readme_path"] for k, v in config["keywords"].items()}
    ...
    for topic, query in keywords.items():
        ...
        if config["publish_gitpage"]:
            ...
            json_to_md(
                json_file, gitpage_path, task=f"Update GitPage {topic}", to_web=True
            )
